 ...
 */
namespace PGMB\Vendor\Mustache;

use PGMB\Vendor\Mustache\Cache\FilesystemCache;
use PGMB\Vendor\Mustache\Cache\NoopCache;
use PGMB\Vendor\Mustache\Exception\InvalidArgumentException;
use PGMB\Vendor\Mustache\Exception\RuntimeException;
use PGMB\Vendor\Mustache\Exception\UnknownTemplateException;
use PGMB\Vendor\Mustache\Loader\ArrayLoader;
use PGMB\Vendor\Mustache\Loader\MutableLoader;
use PGMB\Vendor\Mustache\Loader\StringLoader;
...
class Engine
{
     ...
     *         'loader' => new \PGMB\Vendor\Mustache\Loader\FilesystemLoader(__DIR__.'/views'),
     *
     *         // A Mustache loader instance for partials.
     *         'partials_loader' => new \PGMB\Vendor\Mustache\Loader\FilesystemLoader(__DIR__.'/views/partials'),
     *
     ...
     *         // available as well:
     *         'logger' => new \PGMB\Vendor\Mustache\Logger\StreamLogger('php://stderr'),
     *
     *
     ...
     *         // templates.
     *         'pragmas' => [\PGMB\Vendor\Mustache\Engine::PRAGMA_FILTERS],
     *
     *
     ...
     *
     * @see \PGMB\Vendor\Mustache\Engine::loadTemplate
     * @see PGMB\Vendor\Mustache\Template::render
     *
    ...
    {
        return ['dynamic_names' => $this->dynamicNames, 'inheritance' => $this->inheritance, 'lambdas' => $this->lambdas];
    }
    /**
     ...
     *
     * @see \PGMB\Vendor\Mustache\Engine::setHelpers
     *
     ...
     *
     * @see \PGMB\Vendor\Mustache\Engine::getCache
     *
     * @return Cache
     */
    ...
    public function getTemplateClassName($source)
    {
        ...
        $chunks = ['charset' => $this->charset, 'delimiters' => $this->delimiters ?: '{{ }}', 'entityFlags' => $this->entityFlags, 'escape' => isset($this->escape) ? 'custom' : 'default', 'key' => $source instanceof Source ? $source->getKey() : 'source', 'options' => $this->getOptions(), 'pragmas' => $this->getPragmas(), 'strictCallables' => $this->strictCallables, 'version' => self::VERSION];
    ...
    public function loadPartial($name)
    {
        try {
            ...
            $this->log(Logger::WARNING, 'Partial not found: "{name}"', ['name' => $e->getTemplateName()]);
        }
    }
    /**
     ...
     *
     * @see \PGMB\Vendor\Mustache\Engine::loadTemplate
     * @see PGMB\Vendor\Mustache\Engine::loadPartial
     * @see PGMB\Vendor\Mustache\Engine::loadLambda
     *
     * @param string|Source $source
    ...
    private function loadSource($source, $cache = null)
    {
        ...
        if (!isset($this->templates[$className])) {
            ...
            $this->log(Logger::DEBUG, 'Instantiating template: "{className}"', ['className' => $className]);
     ...
     *
     * @see \PGMB\Vendor\Mustache\Tokenizer::scan
     *
     * @param string $source
     *
     * @return array Tokens
     ...
     *
     * @see \PGMB\Vendor\Mustache\Parser::parse
     *
     * @param string $source
     *
     ...
     *
     * @see \PGMB\Vendor\Mustache\Compiler::compile
     *
     * @param string|Source $source
     *
