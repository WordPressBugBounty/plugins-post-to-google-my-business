...
use PGMB\Vendor\Mustache\Exception\SyntaxException;
...
class Compiler
{
    ...
    const KLASS = '<?php
        ...
        class %s extends \\PGMB\\Vendor\\Mustache\\Template
        {
            private $lambdaHelper;%s%s

            public function renderInternal(\\PGMB\\Vendor\\Mustache\\Context $context, $indent = \'\')
            {
                $this->lambdaHelper = new \\PGMB\\Vendor\\Mustache\\LambdaHelper($this->mustache, $context);
                $buffer = \'\';
        %s
    ...
    const SECTION = '
        private function section%s(\\PGMB\\Vendor\\Mustache\\Context $context, $indent, $value)
        {
            $buffer = \'\';
            ...
            if (%s) {
                ...
                if ($value instanceof \\PGMB\\Vendor\\Mustache\\RenderedString) {
    ...
    const FILTER = '
        ...
        if (!(%s)) {
            throw new \\PGMB\\Vendor\\Mustache\\Exception\\UnknownFilterException(%s);
        }
     ...
     *
     * @see \PGMB\Vendor\Mustache\Context::find
     * @see \PGMB\Vendor\Mustache\Context::findDot
     * @see \PGMB\Vendor\Mustache\Context::last
     *
     * @param string $id Variable name
     *
