<?php
/**
 * Unit tests for PGMB_Vendor_WP_Background_Process.
 *
...
class Test_WP_Background_Process extends WP_UnitTestCase {
	/**
	 * Instance of PGMB_Vendor_WP_Background_Process
	 *
	 * @var MockObject|PGMB_Vendor_WP_Background_Process|(PGMB_Vendor_WP_Background_Process&MockObject)
	 */
	protected $wpbp;
	...
	public function setUp(): void {
		...
		$this->wpbp = $this->getMockForAbstractClass( PGMB_Vendor_WP_Background_Process::class );
	...
	private function getWPBPProperty( string $name ) {
		try {
			$property = new ReflectionProperty( 'PGMB_Vendor_WP_Background_Process', $name );
		} catch ( Exception $e ) {
	...
	protected function executeWPBPMethod(string $name, ...$args ) {
		try {
			$method = new ReflectionMethod( 'PGMB_Vendor_WP_Background_Process', $name );
	...
	public function test_push_to_queue() {
		$this->assertClassHasAttribute( 'data', 'PGMB_Vendor_WP_Background_Process', 'class has data property' );
	...
	public function test_maybe_handle_cancelled() {
		...
		$this->wpbp->save();
		$this->assertCount( 2, $this->wpbp->get_batches() );
		update_site_option( $this->executeWPBPMethod( 'get_status_key' ), PGMB_Vendor_WP_Background_Process::STATUS_CANCELLED );
