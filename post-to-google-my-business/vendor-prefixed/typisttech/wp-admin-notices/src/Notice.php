...
{
    const HTML_CLASSES = [self::UPDATE_NAG => 'update-nag', self::ERROR => 'notice notice-error', self::WARNING => 'notice notice-warning', self::INFO => 'notice notice-info', self::SUCCESS => 'notice notice-success'];
    /**
    ...
    {
        printf('<div id="%1$s" class="%2$s">%3$s</div>', esc_attr($this->getHandle()), esc_attr($this->htmlClass), wp_kses_post($this->content));
    }
}