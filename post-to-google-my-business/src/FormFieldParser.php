...
use PGMB\Vendor\Mustache\Engine as MustacheEngine;
class FormFieldParser {
    ...
    private ?MustacheEngine $mustache = null;

    public function __construct( MustacheContextFactory $mustache_context_factory ) {
        $this->mustache_context_factory = $mustache_context_factory;
    }

    public function set_raw_fields( array $form_fields ) {
    ...
    public function sanitize_from_form( array $form_fields ) {
     ...
     * @param $link_parsing_mode
     *
     * @return string
     */
    private function maybe_parse_html( string $content, $link_parsing_mode ) : string {
        /*
         * Strip shortcodes.
         * strip_shortcodes() is deliberately -not- used here because it will delete all the content within the shortcode,
         * which we don't want because it will basically clear all content in for example a Divi page
         */
        $content = preg_replace( "~(?:\\[/?)[^\\]]+/?\\]~s", '', $content );
        //do not process further if there is no HTML
        ...
        $parse_html = new Html2Text($content, array(
            'width'    => 0,
            'do_links' => $link_parsing_mode,
        ));
        return $parse_html->getText();
    }

    private function get_mustache_instance() : MustacheEngine {
        if ( $this->mustache instanceof MustacheEngine ) {
            return $this->mustache;
        }
        $this->mustache = new MustacheEngine([
            'escape'  => function ( $value ) {
                return $this->maybe_parse_html( $value, $this->get_link_parsing_mode() );
            },
            'helpers' => [
                'upper'   => fn( $text, $render ) => mb_strtoupper( $render( $text ) ),
                'lower'   => fn( $text, $render ) => mb_strtolower( $render( $text ) ),
                'ucfirst' => fn( $text, $render ) => ucfirst( $render( $text ) ),
                'oneline' => function ( $text, $render ) {
                    return preg_replace( '/\\s+/', ' ', trim( $render( $text ) ) );
                },
            ],
        ]);
        return $this->mustache;
    ...
    public function getLocalPost( Location $location, $parent_post_id ) : LocalPost {
        ...
        $mustache = $this->get_mustache_instance();
        $context = $this->mustache_context_factory->build_for_post( $parent_post_id, $location );
        ...
        $summary = $this->parse_placeholder_variables( $placeholder_variables, $mustache->render( $summary, $context ) );
        $summary = $mustache->render( $summary, $context );
        //		$summary = $this->parse_rich_content($summary);
        ...
        //Add image/video
        $customUrl = ( !empty( $this->form_fields['mbp_post_attachment'] ) ? $mustache->render( $this->form_fields['mbp_post_attachment'], $context ) : "" );
        $mediaItem = $this->get_media_item( $parent_post_id, $customUrl );
        if ( !empty( $mediaItem ) && $topicType !== 'ALERT' ) {
            //Alert posts don't have images
        ...
        if ( $topicType == 'OFFER' || $topicType == 'EVENT' ) {
            ...
            $eventTitle = $this->parse_placeholder_variables( $placeholder_variables, $mustache->render( $eventTitle, $context ) );
            $startdate = new \DateTime($this->parse_placeholder_variables( $placeholder_variables, $mustache->render( $this->form_fields['mbp_event_start_date'], $context ) ), DateTimeCompat::get_timezone());
            $enddate = new \DateTime($this->parse_placeholder_variables( $placeholder_variables, $mustache->render( $this->form_fields['mbp_event_end_date'], $context ) ), DateTimeCompat::get_timezone());
    ...
    public function get_media_item( $parent_post_id, $custom_image_url = "" ) {
        ...
        if ( !empty( $custom_image_url ) ) {
            ...
            if ( $image_id && wp_attachment_is_image( $image_id ) ) {
                ...
                $url = $this->validate_external_image_size( $custom_image_url );
            }
